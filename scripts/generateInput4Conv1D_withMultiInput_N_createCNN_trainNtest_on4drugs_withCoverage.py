...
class baseHparamsNvars(object):
    ...
    def __init__(
        ...
        # log_path='CNN1D_withCov.log',
        log_path="CNN1D_withTotalCov.log",
...
def generate_multiInputsNlabels4CNN(hparams, drug):
    ...
    for i, sra in enumerate(sra_list):
        ...
        if not (os.path.isfile(report_path)):
            ...
            var_input_fromOneSample = generate_var_inputFromOneIsolate(
...
def generate_var_inputFromOneIsolate(i, df, report_df, sra, hparams, drug):
    ...
    for var in hparams.variants[drug]:
        ...
        nor_acc = np.zeros((1, hparams.window_size, len(_ALLOWED_BASES) + 1))

        # if (not(var in report_df['var_ID'].values)) and df.loc[i,var]==1:
        # f.write('{} does not exist in {}, however it exists in feature matrix\n'.format(var, sra))
        # f_sra_false_vc.write('{}\n'.format(sra))
        if (var in report_df["var_ID"].values) and df.loc[i, var] == 1:
            # if var in report_df['var_ID'].values: #Fill in the variants presenting in report, no matter wheather the alignments meet match criteria or not
            v_index = 10000
            ...
            for locus in range(start, end):
                if locus in df_depth.loc[df_depth["ctg"] == ctg, "loc"]:
                    ...
                    if df_re["alt"].iloc[0] == ".":
                        ...
                        cov = int(df_re["c_4base"].iloc[0])
                        nor_cov = (cov - _MIN_COV) / (
                            _MAX_COV - _MIN_COV
                        )  # normalize coverage
                        nor_acc[0, (locus - start), 4] = nor_cov
                    else:
                        ...
                        # nor_cov=(base_count[0]-_MIN_COV)/(_MAX_COV-_MIN_COV)
                        nor_cov = (t_depth - _MIN_COV) / (_MAX_COV - _MIN_COV)
                        nor_acc[0, (locus - start), 4] = nor_cov
                        if len(alt_base) != len(base_count):
                            f.write(
                                "# of bases does not match # of counts for {} in {}, {}, {}".format(
                                    var, sra, ctg, v_locus
                                )
                            )
                        else:
...
def build_model(hparams, drug):
    ...
    for i in range(l_v):
        digit[i] = Input(shape=(hparams.window_size, len(_ALLOWED_BASES) + 1))
        x[i] = Conv1D(
            filters=32,
            kernel_size=5,
    ...
    out = Dense(units=32, activation=tf.nn.relu, kernel_regularizer=l2_reg(hparams.l2))(
        concatenated
    )
    out = Dropout(rate=0.3)(out)
    out = Dense(units=16, activation=tf.nn.relu, kernel_regularizer=l2_reg(hparams.l2))(
        out
    )
    out = Dropout(rate=0.3)(out)
...
def run(hparams, trainX, trainY, testX, testY, drug, seed=1):
    """Creates a model, run training and evaluation on one of n-fold."""
